...
declare namespace Deno {
   ...
   * @category Testing
   */
  export function bench(name: string, options: Omit<BenchDefinition, 'fn' | 'name'>, fn: (b: BenchContext) => void | Promise<void>): void
   ...
   * @category Network
   */
  export function resolveDns(query: string, recordType: RecordType, options?: ResolveDnsOptions): Promise<string[] | CAARecord[] | MXRecord[] | NAPTRRecord[] | SOARecord[] | SRVRecord[] | string[][]>
...
declare namespace Deno {
   ...
   * @category FFI
   */
  export type NativeType = NativeNumberType | NativeBigIntType | NativeBooleanType | NativePointerType | NativeBufferType | NativeFunctionType | NativeStructType
   ...
   * @category FFI
   */
  type ToNativeType<T extends NativeType = NativeType> = T extends NativeStructType
    ? BufferSource
   ...
   * @category FFI
   */
  type ToNativeResultType<T extends NativeResultType = NativeResultType> = T extends NativeStructType
    ? BufferSource
    : T extends NativeNumberType
    ? T extends NativeU8Enum<infer U>
      ? U
   ...
   * @category FFI
   */
  type FromNativeResultType<T extends NativeResultType = NativeResultType> = T extends NativeStructType
    ? Uint8Array
    : T extends NativeNumberType
    ? T extends NativeU8Enum<infer U>
      ? U
   ...
   * @category FFI
   */
  export interface ForeignFunction<Parameters extends readonly NativeType[] = readonly NativeType[], Result extends NativeResultType = NativeResultType, NonBlocking extends boolean = boolean> {
    /** Name of the symbol.
     *
   ...
   * @category FFI
   */
  export interface UnsafeCallbackDefinition<Parameters extends readonly NativeType[] = readonly NativeType[], Result extends NativeResultType = NativeResultType> {
   ...
   * @category FFI
   */
  type UnsafeCallbackFunction<Parameters extends readonly NativeType[] = readonly NativeType[], Result extends NativeResultType = NativeResultType> = Parameters extends readonly []
   ...
   * @category FFI
   */
  export class UnsafeCallback<Definition extends UnsafeCallbackDefinition = UnsafeCallbackDefinition> {
    constructor(definition: Const<Definition>, callback: UnsafeCallbackFunction<Definition['parameters'], Definition['result']>)
     ...
     */
    static threadSafe<Definition extends UnsafeCallbackDefinition = UnsafeCallbackDefinition>(definition: Const<Definition>, callback: UnsafeCallbackFunction<Definition['parameters'], Definition['result']>): UnsafeCallback<Definition>
  ...
  /** @category FFI */
  type Const<T> = Cast<T, (T extends string | number | bigint | boolean ? T : never) | { [K in keyof T]: Const<T[K]> } | []>
   ...
   * @category FFI
   */
  export function dlopen<S extends ForeignLibraryInterface>(filename: string | URL, symbols: Const<S>): DynamicLibrary<S>
   ...
   * @category HTTP Server
   */
  export function upgradeHttp(request: Request): Promise<[Deno.Conn, Uint8Array]>
   ...
   * @category KV
   */
  export type KvListSelector = { prefix: KvKey } | { prefix: KvKey; start: KvKey } | { prefix: KvKey; end: KvKey } | { start: KvKey; end: KvKey }
   ...
   * @category KV
   */
  export type KvMutation = { key: KvKey } & ({ type: 'set'; value: unknown; expireIn?: number } | { type: 'delete' } | { type: 'sum'; value: KvU64 } | { type: 'max'; value: KvU64 } | { type: 'min'; value: KvU64 })
  ...
  export class AtomicOperation {
     ...
     */
    enqueue(value: unknown, options?: { delay?: number; keysIfUndelivered?: Deno.KvKey[] }): this
    /**
  ...
  export class Kv {
     ...
     * ```
     */
    enqueue(value: unknown, options?: { delay?: number; keysIfUndelivered?: Deno.KvKey[] }): Promise<KvCommitResult>
  ...
  export namespace jupyter {
     ...
     *
     * @category Jupyter */
    export function broadcast(msgType: string, content: Record<string, unknown>): Promise<void>
  }
}
