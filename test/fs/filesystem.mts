import { Test, Path, Assert, Crypto, Fs, Buffer, Value } from '@sinclair/carbon'
...
Test.describe('Fs:FileSystem', () => {
  ...
  Test.it('Should write and stat', async () => {
    ...
    const stat = (await fs.stat(path)) as Fs.FileStat
  ...
  Test.it('Should stat directory', async () => {
    ...
    await fs.mkdir(path)
    const stat = (await fs.stat(path)) as Fs.DirectoryStat
  ...
  Test.it('Should write and read', async () => {
    ...
    Assert.isEqual(Value.Hash(input), Value.Hash(output))
  })
  ...
  Test.it('Should write then read range', async () => {
    ...
    async function range(start: number, end: number) {
      ...
      const hash1 = Value.Hash(range1)
      const hash2 = Value.Hash(range2)
